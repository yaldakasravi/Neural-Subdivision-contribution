...
def compute_cotangent_weights(vertices, faces):
    ...
    cot_weights = torch.zeros((n, n),device=device)  # Sparse representation could be used for efficiency
    for face in faces:
        i, j, k = face
        vi, vj, vk = vertices[i], vertices[j], vertices[k]
        e_ij, e_jk, e_ki = vj - vi, vk - vj, vi - vk
        
        cot_alpha = torch.dot(e_ij, -e_ki) / torch.linalg.norm(torch.cross(e_ij, -e_ki))
        cot_beta = torch.dot(e_jk, -e_ij) / torch.linalg.norm(torch.cross(e_jk, -e_ij))
        cot_gamma = torch.dot(e_ki, -e_jk) / torch.linalg.norm(torch.cross(e_ki, -e_jk))
        
        cot_weights[i, j] += cot_alpha / 2
        cot_weights[j, i] += cot_alpha / 2
        cot_weights[j, k] += cot_beta / 2
        cot_weights[k, j] += cot_beta / 2
        cot_weights[k, i] += cot_gamma / 2
        cot_weights[i, k] += cot_gamma / 2
...
def compute_vertex_areas(vertices, faces):
    n = len(vertices)
    device = vertices.device
    areas = torch.zeros(n,device=device)
    for face in faces:
        i, j, k = face
        vi, vj, vk = vertices[i], vertices[j], vertices[k]
        area = torch.linalg.norm(torch.cross(vj - vi, vk - vi)) / 6.0
        areas[i] += area
        areas[j] += area
        areas[k] += area
    return areas
...
def compute_mean_curvature(vertices, faces):
    cot_weights = compute_cotangent_weights(vertices, faces)
    vertex_areas = compute_vertex_areas(vertices, faces)
    
    device = vertices.device
    n = len(vertices)
    laplacian = torch.zeros((n, 3),device=device)
    for i in range(n):
        neighbors = cot_weights[i].nonzero(as_tuple=True)[0]
        for j in neighbors:
            laplacian[i] += cot_weights[i, j] * (vertices[j] - vertices[i])
    
    laplacian /= vertex_areas.unsqueeze(1)  # Divide by area for normalization
    mean_curvature = 0.5 * torch.linalg.norm(laplacian, dim=1)
    return mean_curvature

class Mesh: 
    # store information of a mesh
    def __init__(self, V, F, hfList,curves):
        """
        Inputs:
            V: nV-3 vertex list
            F: nF-3 face list
...
def processTrainShapes(folder):
    ...
    # load data
    #NOTE added new fucntionality to support to read reading curvature
    meshes = [None] * nObjs
    for ii in range(nObjs):
        ...
        for jj in range(nSubd + 1):
            ...
            print(f" processing object number {ii} and subdivion {jj}")
            _, hfList = computeFlapList(V, F, 1)

            X_tmp = compute_mean_curvature(V,F)
            meshes_i[jj] = Mesh(V, F, hfList[0],X_tmp)
...
class TrainMeshes: 
    ...
    def compute_curve(self):
        #NOTE Yalda
        #NOTE computing the curvature if it needs to be directed compuputed , 
        nM = self.nM 
        nS = self.nS # number of subdivision levels
        curves =  [None] * nM
        for ii in range(nM):
            curves_ij = [None] * nS
            for jj in range(nS):
                vertices = self.meshes[ii][jj].V
                faces = self.meshes[ii][jj].F
                print(" vertices {vertices.shape} faces {faces.shape}")
                print(f" ii {ii} jj {jj} ")
                X_tmp = compute_mean_curvature(vertices, faces)
                mx = torch.max(X_tmp)
                mn = torch.min(X_tmp)

                curves_ij[jj] = (X_tmp - mn)/ ( mx-mn)
                #print(mx,mn)
                #print(curves_ij[jj].size)
                #print(torch.max(curves_ij[jj]),torch.min(curves_ij[ii]))
                #input()
                #curves_ij[jj] = self.meshes[ii][jj].V[:,0]
            curves[ii] = list(curves_ij) # one-ring pooling matrix
        return curves
    def get_curve(self):
        #NOTE Yalda
        #NOTE  access precomputed curvature information 
        nM = self.nM 
        nS = self.nS # number of subdivision levels
        curves =  [None] * nM
        for ii in range(nM):
            ...
            for jj in range(nS):
                ...
                faces = self.meshes[ii][jj].F
                #X_tmp = compute_mean_curvature(vertices, faces)
                mx = torch.max(self.meshes[ii][jj].C)
                mn = torch.min(self.meshes[ii][jj].C)

                curves_ij[jj] = (self.meshes[ii][jj].C - mn)/ ( mx-mn)
                #print(mx,mn)
                #print(curves_ij[jj].size)
                #print(torch.max(curves_ij[jj]),torch.min(curves_ij[ii]))
                #input()
                curves_ij[jj] = self.meshes[ii][jj].V[:,0] * 0
            curves[ii] = list(curves_ij) # one-ring pooling matrix
        return curves
    def toDevice(self, device):
        ...
        for ii in range(self.nM):
            for jj in range(self.nS):
                ...
                self.meshes[ii][jj].C = self.meshes[ii][jj].C.to(device)
        for ii in range(self.nM):
...
def preprocessTestShapes(meshPathList, nSubd=2):
    ...
    for meshIdx in range(nObjs):
        ...
        for jj in range(nSubd + 1):
            if jj == 0:
                meshes_i[jj] = Mesh(V, FList[jj], hfList[jj],X_tmp)
            else:
                meshes_i[jj] = Mesh(None, FList[jj], hfList[jj],X_tmp)
...
class TestMeshes:
    ...
    def compute_curve(self):
        #NOTE Yalda
        #NOTE  compute curvature for test shapes
        nM = self.nM 
        nS = self.nS # number of subdivision levels
        curves =  [None] * nM
        for ii in range(nM):
    ...
    def get_curve(self):
        #NOTE Yalda
        #NOTE  access pre computed curvature for test shapes
        nM = self.nM 
        nS = self.nS # number of subdivision levels
        curves =  [None] * nM
        for ii in range(nM):
